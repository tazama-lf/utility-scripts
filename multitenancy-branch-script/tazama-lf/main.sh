...
while IFS= read -r repo; do
  ...
  echo "Processing repository: tazama-lf/$repo..."

  # Check if the dev branch exists
  dev_branch_check=$(curl -s -H "Authorization: Bearer $GH_TOKEN" \
    https://api.github.com/repos/tazama-lf/$repo/branches/dev | jq -r '.name')

  if [ "$dev_branch_check" != "dev" ]; then
    echo "❌ Error: 'dev' branch not found in tazama-lf/$repo"
    continue
  fi
  ...
  dev_sha=$(curl -s -H "Authorization: Bearer $GH_TOKEN" \
    https://api.github.com/repos/tazama-lf/$repo/branches/dev | jq -r '.commit.sha')

  # Create the new branch
  response=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
    -H "Authorization: Bearer $GH_TOKEN" \
    -H "Content-Type: application/json" \
    https://api.github.com/repos/tazama-lf/$repo/git/refs \
    -d "{\"ref\": \"refs/heads/multitenancy\", \"sha\": \"$dev_sha\"}")

  if [ "$response" -eq 201 ]; then
    echo "✅ Successfully created 'multitenancy' branch in tazama-lf/$repo"
  else
    echo "❌ Error: Failed to create 'multitenancy' branch in tazama-lf/$repo (HTTP $response)"
  fi
